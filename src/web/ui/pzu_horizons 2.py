            ...
            if prediction_result['status'] == 'success':
                ...
                pred_cards = [
                    kpi_card(
                        ...
                        f"Next {forecast_days} days • {summary['confidence_level'].title()} confidence"
                    ),
                    kpi_card(
                        "Predicted Revenue",
                        format_currency(summary['total_predicted_revenue_eur'], decimals=0, thousands=thousands_sep),
                        "Total energy sales forecast"
                    ),
                    kpi_card(
                        "Avg Daily Profit",
                        format_currency(summary['avg_daily_profit_eur'], decimals=0, thousands=thousands_sep),
                        "Expected daily average"
                    ),
                    kpi_card(
                        "Energy Volume",
                        f"{summary['total_predicted_energy_mwh']:,.0f} MWh",
                        "Forecasted trading volume"
                    )
                ]
