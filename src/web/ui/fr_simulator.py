...
def render_frequency_regulation_simulator(
    ...
    if export8_path:
        try:
            ...
            if not imb_df.empty and any(v.get('enabled') and v.get('mw', 0) > 0 for v in products_cfg.values()):
                ...
                # Check if DAMAS activation data is available
                # First, check if imb_df already contains DAMAS columns (embedded in the CSV)
                has_damas_embedded = 'afrr_up_activated_mwh' in imb_df.columns and 'afrr_down_activated_mwh' in imb_df.columns

                if has_damas_embedded:
                    # DAMAS columns already present in imb_df - no merge needed!
                    use_damas = True
                    damas_coverage = (imb_df['afrr_up_activated_mwh'].notna()).mean()
                    st.info(f"✅ DAMAS activation columns already present in imbalance data (embedded mode)")
                else:
                    # Try to merge from separate DAMAS file
                    ...
                    if use_damas:
                        try:
                            ...
                            damas_coverage = (imb_df['afrr_up_activated_mwh'].notna()).mean()
                            st.info(f"✅ DAMAS activation data merged from separate file")
                ...
                if use_damas:
                    try:
                        # Count how many slots have DAMAS data (already set above if embedded)
                        if not has_damas_embedded:
                            damas_coverage = (imb_df['afrr_up_activated_mwh'].notna()).mean()
    ...
    if fr_metrics and 'months' in fr_metrics and len(fr_metrics['months']) > 0:
        ...
        for month_rec in months_data:
            ...
            energy_cost = month_rec.get('energy_cost_eur', 0.0)
                ...
                if '-' in month_str:
                    ...
                    for day in range(1, days_in_month + 1):
                        daily_records.append({
                            ...
                            'energy_cost_eur': energy_cost / days_in_month,
                        })
            except Exception:
                continue
        ...
        if len(daily_records) >= 30:
            ...
            if st.button("🚀 Generate FR Predictions", key="fr_predict_btn"):
                with st.spinner("Training ML models and generating predictions..."):
                    ...
                    daily_opex = fr_operating_cost_annual / 365.0
            ...
            if 'fr_predictions' in st.session_state and 'fr_prediction_summary' in st.session_state:
                ...
                # Calculate total operating cost for forecast period
                total_operating_cost = fr_operating_cost_annual * (forecast_days_fr / 365.0)

                cards = [
                    kpi_card(
                        "Predicted Net Profit",
                        f"€{summary['total_profit_eur']:,.0f}",
                        f"After all costs"
                    ),
                    kpi_card(
                        "Predicted Total Revenue",
                        f"€{summary['total_revenue_eur']:,.0f}",
                        f"€{summary['avg_daily_revenue_eur']:,.0f}/day"
                    ),
                    kpi_card(
                        "Total Energy Cost",
                        f"€{summary['total_energy_cost_eur']:,.0f}",
                        f"€{summary['avg_daily_energy_cost_eur']:,.0f}/day"
                    ),
                    kpi_card(
                        "Total Operating Cost",
                        f"€{total_operating_cost:,.0f}",
                        f"€{fr_operating_cost_annual / 365.0:,.0f}/day"
                    ),
                ]
                ...
                with st.expander("📋 Detailed Daily Predictions"):
                    ...
                    display_df.columns = [
                        ...
                        'Energy Cost (EUR)', 'Operating Cost (EUR)', 'Net Profit (EUR)'
                    ]
